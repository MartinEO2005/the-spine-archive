...
const SpineGrid = ({ spines, selectedSpines, toggleSpine, hoveredId, setHoveredId }) => {
  return (
    <div style={{ flex: 1, overflowY: 'auto', padding: '50px', display: 'flex', flexWrap: 'wrap', gap: '30px', justifyContent: 'center' }}>
      {spines.map(spine => {
        ...
        return (
          <div key={spine.id} onMouseEnter={() => setHoveredId(spine.id)} onMouseLeave={() => setHoveredId(null)} onClick={() => toggleSpine(spine)}
            ...
            <img src={spine.src} alt={spine.title} loading="lazy" style={{ height: '100%', width: 'auto', display: 'block', borderRadius: '4px', border: isSelected ? '4px solid #00ff00' : '1px solid #333' }} />
            {isHovered && (
              <div style={{ position: 'absolute', bottom: '-50px', left: '50%', transform: 'translateX(-50%)', backgroundColor: 'rgba(0,0,0,0.9)', color: 'white', padding: '8px 15px', borderRadius: '4px', fontSize: '12px', whiteSpace: 'nowrap', fontWeight: 'bold', border: '1px solid #b30000', zIndex: 20 }}>
